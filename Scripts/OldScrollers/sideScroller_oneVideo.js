...
function VideoPlayer() {
	...
	var video = document.getElementsByClassName('fullDitty')[0];

	this.setSource = function(dittyType) {
		if(video.src.indexOf(dittyType) === -1){	
				var currentPlayingTime = video.currentTime;
				var v="Videos/" + dittyType;
				video.src='';
				video.src=v;    
				//video.src = "Videos/" + dittyType;
				video.load();
				video.addEventListener('loadedmetadata', function () {
					video.currentTime = currentPlayingTime;
    				}, false);
		}
	};
	...
	this.initPlayback = function () {
		...
		addEventListenerOnce(video, "playing", function(){ //start playback
			console.log("video playing");
			addEventListenerUntilTrue(video,"timeupdate", function(){
				console.log("video was updated:" + video.currentTime );
				// Calculate Duration
				if(playbackStart === false){
					playbackStart = true;
					timer = new Timer(function(){
						processNextAnimation(video);
					}, playBackDelay - (video.currentTime * 1000));
				}
			});
		});
		*/
		
		//$(".fullDitty").fadeIn(500);
		videoPlaybackState = "Pause";
		$('.fa').hide();
