	...
	//turns on advanced player controls
	var playControls = true;
	var currentGateNumber = 0;
...
$(document).ready(function () {	
	...
	$.getJSON("/References/Quirky.json", function(json) { //Yama_NorthCack.json //DeLaAnimation.json //Quirky.json
			// Load the first interaction: video files and audio
			segmentDict = json.interactiveSegments;
			playControls = json.playerControls;
			loadInteraction(json.players, json.songSelection);
			obstaclesDic = json.obstacles;
			playerDic = json.players;
			roster.registerPlayers(json.players);
			environment = navigation.currentEnvironment();
		});
...
function loadInteraction(videoInput, songInput){
	...
	validateVideos(videoInput); // videoInput is playerInformation + videos
	
	//Generate Videos
	//generateVideos(videoInput);
	$('#section1')[0].src = "Videos/" + videoInput[0].startVideo;
	$('#section1')[0].load();
	
	$('#section0')[0].src = "Videos/" + videoInput[1].startVideo;
	$('#section0')[0].load();
	
	$('#playerIcon')[0].src = "/Images/Players/" + videoInput[0].image;
	$('#playerName')[0].innerHTML ='<h2>' + videoInput[0].name + '</h2>';
...
function checkObstacleInteraction(time){
	var remove = false;
		// looks through obstacle dictionary and issues call to update timestamp if needed.
	...
	if(remove){
		...
		// update the player logic for succeed/loss
		if(playControls){
...
function Roster(){
	...
	// change the title namecard
	this.switch = function(index){
		//some hacky ass shit cause I'm 1 up
		index--;
		if(index < 0){
			index += navigation.getEnvironmentSize();
		}
		
		var currentPlayer = navigation.getPlayerAt(index);  // playerDic[index]				
		playerIcon.src = "Images/Players/" +currentPlayer.image;
		playerName.innerHTML = "<h2>" + currentPlayer.name + "</h2>";		
	};
	
	// create player objects which control the videos
	this.registerPlayers = function(playerInformation){
...
function VideoPlayer() {
	...
	// Called when a background video needs to update based on obstacle
	this.resetBackgroundSource = function(){
		// fetch the last video on the queue
		var backgroundVideo = videoQueue.pop();
		
		// I have to cheat and get video source from title
		var elementSource = backgroundVideo.src;
		var elementArray = elementSource.split("/");
		var newIndex = elementArray.pop().charAt(0);
		backgroundVideo.src = "Videos/" + navigation.getVideoAt(newIndex);
		
		// Load video.
		backgroundVideo.load();
		backgroundVideo.play();
		videoQueue.push(backgroundVideo);
		
		backgroundVideo.addEventListener('loadedmetadata', function () {
			var timeA = $("#" + videoQueue[1].id)[0].currentTime;
			var timeB = $("#" + videoQueue[0].id)[0].currentTime;
			console.log("a: " + timeA + " b:" + timeB);
					$("#" + videoQueue[1].id)[0].currentTime = $("#" + videoQueue[0].id)[0].currentTime;
					if(maxTime > backgroundVideo.duration){
						maxTime = backgroundVideo.duration;
					}
    					}, false);
...
function Navigation(){
	var environment = [];
	var pagination = 1; // because the first two videos are loaded, the NEXT one would be #2
	var init = false;
	...
	// Gets the current video at given index
	this.getVideoAt = function(index){
		return environment[index].startVideo;
	};
	
	this.getEnvironmentSize = function(){
	...
	this.currentEnvironment = function(){
		return environment[pagination].startVideo;
	};
	
	// increase the page - returns video
	...
	// decrement the page - returns video
	this.decrement = function(){
		pagination--;
		if(pagination < 0){
			pagination += environment.length;
	...
	this.validateNavigation = function(){
		...
		// find previous index
		var previousIndex = pagination; //current video index
		previousIndex--;
		if(previousIndex < 0){
			previousIndex += environment.length;
		}
			
		// check to see if the player posseses the correct key to pass the gate.
		var gateKeyList = environment[previousIndex].gateKeys;
		if(gateKeyList.indexOf(currentGateNumber) === -1){
			//Game over, player could not pass gate
			console.log("LOSE");
			gameEngine.kill();
		}
		
		// Player passed gate, issue rewards (if applicable)
		var rewardKey = environment[previousIndex].obstacleRewards[currentGateNumber];
		
		// If there exist a reward key, add it to the player dictionary.
		if(rewardKey > 0){
			environment[previousIndex].gateKeys.push(rewardKey);
			environment[previousIndex].rewardCount++;
			console.log(environment[previousIndex].name)
		}	
		
		// update other videos accordingly
		for (var i = 0; i < environment.length; i++){
			/// now validate against previous index
			if(i === previousIndex){
				continue;
			}
			
			//update all other videos by new rules
			var index = environment[i].rewardCount;
			var nextVideos = environment[i].videos[index].video;
			for (var j = 0; j < nextVideos.length; j++){
				//loop through next videos and select the appropriate title
				console.log(nextVideos[j].deathGate + " gate:" + currentGateNumber + " j: " + j);
				if(nextVideos[j].deathGate > currentGateNumber){
					// We've found the appropriate video, reset
					environment[i].startVideo = nextVideos[j].url;
					console.log(nextVideos[j]);
					break;
				}
			}
		}
		...
		currentGateNumber++;
		videoPlayer.resetBackgroundSource();
	};
}
